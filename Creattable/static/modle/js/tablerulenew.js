...
$(document).ready(function () {
    ...
    var url = urlnei + "/work_table_rule?limit=10";
    var pagecount;
        ...
        laypage.render({
            ...
            jump: function(obj, first){ //触发分页后的回调
                ...
                if(!first){ //点击跳页触发函数自身，并传递当前页：obj.curr
                    ...
                    var url = urlnei + "/work_table_rule?limit=10&offset="+offset;
                    DataGet(url, '', function (data) {
                        pagecount = data.count;
                        var msg = data.work_model_list;
                        var temp = TableShow(msg);
...
function TableEdit(obj) {
    ...
    $.cookie('bmid', bmid, {expires: 1, path: '/'});
    window.location.href='infotablerule.html';
}
function TableInfo(obj) {
    var bmid=obj.name;
    $.cookie('bmid', bmid, {expires: -1, path: '/'});
    $.cookie('bmid', bmid, {expires: 1, path: '/'});
    window.location.href='tablerulethis.html';
}
...
var template = '<tr>\n' +
    ...
    '            </tr>';
function newtable() {
    window.location.href='filltablerule.html';
}
