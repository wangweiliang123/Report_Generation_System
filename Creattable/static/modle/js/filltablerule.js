...
var templateselect="     <select class=\"tableselect\" lay-verify=\"{{required}}\">\n" +
    "            </select>";
    ...
    DataGet(urlthis,'',function(data) {
        ...
        for(var i=0;i<msg.step_order.length;i++){
            ...
            if(msg.step_order[i]==5){
                var temp=template6.replace(/{{i}}/g, BgNum).replace(/{{tabletitle}}/g, "").replace(/{{tablehtml}}/g,msg.html_code[BgNum]);
                $("#newtable").append(temp);
                ...
                temp=template7.replace(/{{tablenum}}/g, BgNum).replace(/{{tabletitle}}/g, "控件表格，请按照规则填写！").replace(/{{tablehtml}}/g,msg.html_code[BgNum]);
                $("#newtable").append(temp);
                $("#newtable").find("table").eq(BgNum*3+2).find("td").each(function(){
                    $(this).html("")
                });
                BgNum++;
            }
...
function BtnSave() {
    senddata={
        base_model:tabletemid.toString(),
...
function ShowTable() {
    ...
    tabletemid=taskid;
    if(taskid){
        var url1=urlnei+"/info_table?pk="+taskid;
            $("#showtable").show();
